...
## The two themes for cycle-nodes and -lines
...
**Simplest Working example** — bike nodes in Varde (municipal code = 573), as lat/lon GeoJSON:
```
...
```
https://geofa.geodanmark.dk/api/v2/sql/fkg?q=SELECT+geometri+FROM+fkg.t_5609_cykelknudepunkterr+WHERE+kommunekode=573&srs=4326&format=geojson
```
...
# - DEPRECATED - Connecting to the GeoFA Ecosystem — Bike Nodes & Bike-Node Routes
...
**Important quirk:** 5609 has no `from_node` / `to_node` column. 
The connection between a stretch and its two endpoint nodes has __to be derived spatially__ — 
snap each line's start/end vertex to the nearest 5608 point. 
